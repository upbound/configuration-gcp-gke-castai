# Project Setup
PROJECT_NAME := configuration-gcp-gke-castai
...
# $ export UPTEST_CLOUD_CREDENTIALS=$(echo "GCP='$(cat ~/.gcloud/credentials)'\nCASTAI='$(cat castai.json)'")
uptest: $(UPTEST) $(KUBECTL) $(KUTTL)
	@$(INFO) running automated tests
	@KUBECTL=$(KUBECTL) KUTTL=$(KUTTL) CROSSPLANE_NAMESPACE=$(CROSSPLANE_NAMESPACE) $(UPTEST) e2e examples/network-xr.yaml,examples/gke-xr.yaml --setup-script=test/setup.sh --default-timeout=2400 || $(FAIL)
	@$(OK) running automated tests

e2e: build controlplane.up local.xpkg.deploy.configuration.$(PROJECT_NAME) uptest

# render:
# 	crossplane beta render examples/readonly.yaml apis/readonly/composition.yaml examples/functions.yaml -r
# 	crossplane beta render examples/fullaccess.yaml apis/fullaccess/composition.yaml examples/functions.yaml -r
